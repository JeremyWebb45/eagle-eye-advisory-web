...
def get_file(
    ...
    logger.info(f"Raw filename received: {filename}")
    
    # Decode URL-encoded filename
    filename = unquote(filename)
    logger.info(f"Decoded filename: {filename}")
    logger.info(f"Allowed files: {ALLOWED_FILES}")
    
    # Verify file is in whitelist to prevent directory traversal attacks
    if filename not in ALLOWED_FILES:
        logger.warning(f"File '{filename}' not in whitelist")
        raise HTTPException(
    ...
    logger.info(f"Protected files dir: {PROTECTED_FILES_DIR}")
